...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Download mp3s and process the case jsons into something usable')
    parser.add_argument('--case-folder', type=str,
                        help='Location of the case json files')
...
def process_case(c):
    ...
    if url:  # If mp3 was found
        ...
        if transcription:  # If transcription is valid
            mp3_outfile = os.path.join(
                base_audio_folder, '{}.mp3'.format(rec_name))
            transcript_outfile = os.path.join(
                base_transcript_folder, '{}.json'.format(rec_name))
...
def collate_speakers():
    ...
    assert len(
        transcripts) > 1, 'Could only find 1 or less transcription json files'
    all_speaker_dict = {}

    for t in transcripts:
        js = json.load(open(t, encoding='utf-8'),
...
def mp3_and_transcript_exist(c):
    mp3_exist = os.path.isfile(os.path.join(
        base_audio_folder, '{}.mp3'.format(os.path.splitext(os.path.basename(c))[0])))
    transcript_exist = os.path.isfile(os.path.join(
        base_transcript_folder, '{}.json'.format(os.path.splitext(os.path.basename(c))[0])))
